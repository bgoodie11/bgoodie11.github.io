const canvas=document.getElementById("network-canvas"),ctx=canvas.getContext("2d"),wrap=document.querySelector(".canvas-wrap"),dialog=document.getElementById("node-dialog");
const COLS=18,ROWS=11;
const nodes=[
{id:"yext",label:"YEXT · 24→",gx:3,gy:2,tag:"OCTOBER 2024 — NOW · NEW YORK",title:"Technical Partner Manager",copy:"Yext. Product integrations and technical partnerships."},
{id:"curinos",label:"CURINOS · 24",gx:9,gy:1,tag:"MARCH — OCTOBER 2024",title:"Associate",copy:"Curinos. Financial services data and consulting."},
{id:"zest",label:"ZEST · 21",gx:15,gy:2,tag:"2021 — 2022",title:"Business Development Intern",copy:"Zest. An early-stage startup and my first real look at building a company."},
{id:"crowded",label:"CROWDED · 22",gx:15,gy:8,tag:"2022 · TEL AVIV",title:"Product Management Intern",copy:"Crowded. Fintech product work, customer discovery, and QA."},
{id:"unc",label:"UNC · 20–23",gx:10,gy:9,tag:"2020 — DECEMBER 2023",title:"UNC Kenan-Flagler",copy:"B.S. in Business Administration + Data Science."},
{id:"amazon",label:"AMAZON · 23",gx:3,gy:8,tag:"2023 · SEATTLE",title:"Technical Program Manager Intern",copy:"Amazon. Data management, automation, and operations at scale."},
{id:"scaffold",label:"SCAFFOLDMAX · 26",gx:9,gy:5,tag:"2026 · SIDE PROJECT",title:"ScaffoldMaxNYC",copy:"An interactive NYC development capacity explorer.",link:"/scaffold/"}
];
let width=0,height=0,dpr=1,cell=40,offsetX=0,offsetY=0,tick=0;
let snake=[],dir={x:1,y:0},visited=new Set();
function reset(){snake=[{x:6,y:5},{x:5,y:5},{x:4,y:5},{x:3,y:5}];dir={x:1,y:0};visited=new Set();document.getElementById("visited-count").textContent=0;canvas.focus();draw()}
...
function move(next){dir=next;const head=snake[0],newHead={x:(head.x+dir.x+COLS)%COLS,y:(head.y+dir.y+ROWS)%ROWS};snake.unshift(newHead);snake.length=4+visited.size;const hit=nodes.find(n=>n.gx===newHead.x&&n.gy===newHead.y);if(hit&&!visited.has(hit.id))openNode(hit);draw()}
